...
public class FontObjLoader extends ObjLoader {
    ...
    public FontStillModel loadObj (FileHandle file, boolean flipV) {
     ...
     * @param textureDir
     * @param flipV whether to flip the v texture coordinate (Blender, Wings3D, et al) */
    public FontStillModel loadObj (FileHandle file, FileHandle textureDir, boolean flipV) {
        String path = "";
        ...
        try {
            try {
                return loadObj(file, reader, flipV);
            } catch (IOException e) {
                return null;
            }
        } finally {
    ...
    public List<FontStillModel> loadFont() {
        return null;
    }

    public FontStillModel loadObj(FileHandle file, BufferedReader reader, boolean flipV) throws IOException {
        String line;
        String[] tokens;
        char firstChar;
        ...
        while (line != null && !line.startsWith("#*--")) {
            ...
            if (tokens[0].length() == 0) {
                ...
                try {
                    c = (char)Integer.parseInt(tokens[1]);
                } catch (NumberFormatException ignore) { }
                try {
                    spacing = Integer.parseInt(tokens[2]);
                } catch (NumberFormatException ignore) { }
                try {
                    kern = Integer.parseInt(tokens[3]);
                } catch (NumberFormatException ignore) { }
...
//                }
        ...
        final FontStillModel model = new FontStillModel(c, data);
